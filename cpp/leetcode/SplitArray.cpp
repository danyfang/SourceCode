...
class Solution{
public:
    //solution provided by user@dax1ng
    /**
    ...
    3 Use greedy to narrow down left and right boundaries in binary search.
        ...
        3.3 We'll end up with two results: either we can divide the array into more than m subarrays
        or we cannot.
            ...
            If we can't, it is either we successfully divide the array into m parts and the sum of each
            part is less than mid, or we used up all numbers before we reach m. Both of them mean that
            we should lower mid because we need to find the minimum one. This leads to r = mid - 1;
    */
...
int main(){
    ...
    cin >> part;
    while(part != 0){
        cout << s.splitArray(nums, part) << endl;
        cin >> part;
    }
    return 0;
}
