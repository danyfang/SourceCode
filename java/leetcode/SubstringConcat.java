...
class Solution{
	//following solution exceeded time limit
	public List<Integer> findSubstring(String s, String[] words) {
		List<Integer> list = new ArrayList<>();        
		int m = words.length;
		int n = words[0].length();
		for(int i=0; i<=s.length()-m*n; i++){
			if(isSame(s.substring(i,i+m*n),words)){
				list.add(i);
			}
		}
		return list;
	...
	private boolean isSame(String s, String[] words){
		int n = words[0].length();
		String[] str = new String[s.length()/n];
		for(int i=0; i<s.length()/n; i++){
			str[i] = s.substring(n*i,(i+1)*n);
		}
	...
	//Solution provided by user@shaka.shadows
