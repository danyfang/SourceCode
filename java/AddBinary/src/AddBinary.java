...
class Solution {
    public String addBinary(String a, String b) {
        ...
        for(int i=lon.length()-1; i>=0; i--){
            if(j<0){
                result[i] = Integer.parseInt(String.valueOf(lon.toCharArray()[i])) + flag;
                flag = 0;
                if(result[i] >= 2){
                    result[i] = result[i] - 2;
                    flag = 1;
            ...
            result[i] = Integer.parseInt(String.valueOf(lon.toCharArray()[i])) + Integer.parseInt(String.valueOf(sho.toCharArray()[j])) + flag;
            flag = 0;
            if(result[i] >= 2){
                result[i] = result[i] - 2;
                flag = 1;
...
public class AddBinary{
    public static void main(String[] args){
        ...
        System.out.println(c.equals(s.addBinary(a,b)));
